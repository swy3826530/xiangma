			...
			<uni-list>
				<uni-list-item direction="column" v-for="item in data" :key="item.id">
					...
					<template v-slot:body>
						<view v-if="typeof item.avatar === 'string'" class="content uni-media-box">
							<image v-for="img in 3" :key="img" class="uni-thumb" :src="typeof item.avatar === 'string'?item.avatar : item.avatar[0]"
							 mode="aspectFill"></image>
						</view>
						<view v-else class="content uni-media-box">
							<image v-for="(img,index) in item.avatar" :key="index" class="uni-thumb" :src="img" mode="aspectFill"></image>
						</view>
					</template>
					<!-- 通过footer插槽定义列表底部内容 -->
